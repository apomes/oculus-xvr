...
struct MatchingAlternateEye : BasicVR
{
    ...
    void MainLoop()
    {
        ...
        if (!extraRenderTexture.Init(HMD, Layer[0]->pEyeRenderTexture[0]->SizeW, Layer[0]->pEyeRenderTexture[0]->SizeH))
            return;
            ...
            for (int eye = 0; eye < 2; ++eye)
            {
                if (DIRECTX.Key['1'])
                {
                    ...
                    if (((clock % 4) == 2) && (eye == 0))
                    {
                        ...
                        auto rtv = extraRenderTexture.TexRtv[extraRenderTexture.TextureSet->CurrentIndex];
