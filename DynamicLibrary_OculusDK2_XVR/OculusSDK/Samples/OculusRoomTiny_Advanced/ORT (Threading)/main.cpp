...
{
    ovrHmdDesc desc = ovr_GetHmdDesc(state->hmd);
    unsigned int caps = ovr_GetEnabledCaps(state->hmd);
    return ((caps & desc.AvailableHmdCaps) != 0) ? 0 : 1;
}

ThreadTest(SetEnabledCaps)
{
    ovrHmdDesc desc = ovr_GetHmdDesc(state->hmd);
    ovr_SetEnabledCaps(state->hmd, desc.AvailableHmdCaps & state->rand());
    return 0;
}
...
ThreadTest(GetInputState)
{
    ...
    ovrResult result = ovr_GetInputState(state->hmd, ovrControllerType_All & state->rand(), &inputState);
...
ThreadTest(GetFovTextureSize)
{
    ...
    ovrSizei size = ovr_GetFovTextureSize(state->hmd, eye, fov, 1);
...
ThreadTest(GetRenderDesc)
{
    ...
    ovrEyeType eye = randeye(state->rand) ? ovrEye_Left : ovrEye_Right;
    ovrFovPort fov = {1, 1, 1, 1};
    ovrEyeRenderDesc renderDesc = ovr_GetRenderDesc(state->hmd, eye, fov);
    return (renderDesc.HmdToEyeViewOffset.x != 0) ? 0 : 1;
}
...
ThreadTest(BoolProperties)
{
    ...
    if (state->rand() & 1)
    {
        ovr_GetBool(state->hmd, name, ovrFalse);
    }
    else
    {
        ovr_SetBool(state->hmd, name, (state->rand() & 1) ? ovrTrue : ovrFalse);
    }
...
NoThreadTest(IntProperties)
{
    ...
    if (state->rand() & 1)
    {
        ovr_GetInt(state->hmd, name, 0);
    }
    else
    {
        std::uniform_int_distribution<> randval(0, vals[idx]);
        ovr_SetInt(state->hmd, name, randval(state->rand));
    }
...
ThreadTest(FloatProperties)
{
    ...
    if (state->rand() & 1)
    {
        ovr_GetFloat(state->hmd, name, 0);
    }
    else
    {
        std::uniform_real_distribution<> randval(0, vals[idx]);
        ovr_SetFloat(state->hmd, name, float(randval(state->rand)));
    }
...
ThreadTest(StringProperties)
{
    ...
    if (state->rand() & 1)
    {
        const char *foo = ovr_GetString(state->hmd, name, "");
    }
    else
    {
        std::uniform_int_distribution<> randval(0, int(vals[idx].size() - 1));
        ovr_SetString(state->hmd, name, vals[idx][randval(state->rand)]);
    }
...
ThreadTest(FloatArrayProperties)
{
    ...
    {
        float values[32];
        ovr_GetFloatArray(state->hmd, name, values, arraySize[idx]);
    }
    else
    {
        ...
        ovr_SetFloatArray(state->hmd, name, values, size);
    }
...
struct Threading : BasicVR
{
    ...
    void MainLoop()
    {
        ...
        if (!extraRenderTexture.Init(HMD, 1024, 1024))
            return;
        ...
        for (frameIndex = 0; frameIndex < maxFrames; ++frameIndex)
        {
		    ...
		    // Submit them
		    presentResult = ovr_SubmitFrame(HMD, 0, nullptr, layerHeaders, 2);
